  ...
  do
      ...
      for (;;)
	{
	 ...
	 though SIGCONT has cleared the TASK_STOPPED state.  */
      ...
      /* If we got SIGSTOP here it is a bug of the patch
	 https://bugzilla.redhat.com/show_bug.cgi?id=248532#c5
	 tkill (SIGCONT) above should clear all the pending SIGSTOPs.  */
