/* Try to detach a multithreaded process with all its tasks T (Stopped).
   This testcase variant differs from detach-stopped.c by a workaround of
   a different behavior of RHEL-5 kernel requiring one more SIGSTOP and which
   is not going to be changed to be backward compatible in other aspects.
...
reproduce (void)
{
  ...
  /* This loop is the RHEL-5 specific workaround.  SIGSTOP used for
     PTRACE_DETACH is not reliable in multithreaded situation.  But the tkill
     in advance is enough.  */

  for (child_count = THREADS - 1; child_count >= 0; child_count--)
    {
      errno = 0;
      syscall (__NR_tkill, child[child_count], SIGSTOP);
      assert_perror (errno);
    }
  ...
  for (child_count = THREADS - 1; child_count >= 0; child_count--)
    {
      ...
      errno = 0;
      ptrace (PTRACE_DETACH, child[child_count], NULL, NULL);
      assert_perror (errno);
    }
  sleep (1);
