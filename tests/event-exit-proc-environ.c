    ...
    if (pid == 0) {
        ...
        do {
                ...
                if (PTRACE_EVENT(status) == PTRACE_EVENT_EXIT)
		  {
		    ...
		    /* Test environ too.  On vanilla kernels, environ
		       is affected by the permission problem.  But maps
		       is only affected the same way on exec-shield kernels.  */

                    snprintf(environ, sizeof environ, "/proc/%d/environ", pid);
		    struct stat st;
		    int stat_rc = stat(environ, &st);
		    assert (stat_rc == 0);
		    int status_fd = open(environ, O_RDONLY);
                    if (status_fd == -1)
		      {
			int open_errno = errno;
			printf("%s has uid %u and mode %04o\n",
			       environ, st.st_uid, st.st_mode&07777);
