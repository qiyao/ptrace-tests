...
reproduce (void)
{
  ...
  /* Sending another SIGSTOP to get SIGSTOP reported by waitpid reliably.
     ...
     If we disable this SIGSTOP + PTRACE_CONT hack still SIGSTOP may be stacked
     twice (one stopped and once waiting in the queue).  As we waitpid it only
     once after PTRACE_DETACH (NULL) below the process would remain
     T (Stopped).
  ...
  errno = 0;
  i = syscall (__NR_tkill, child, SIGSTOP);
  assert_perror (errno);
  assert (i == 0);
