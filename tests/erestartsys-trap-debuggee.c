/* Child -m32 built program only to be called by `./erestartsys-trap-debugger'.
...
main (int argc, char **argv)
{
  long l;
  unsigned u;
  /* Its size must match `erestartsys-trap-debugger.c'.  */
  char buf[0x100];
  ssize_t ssize;
  int i;
  void *voidp;
  ...
  if (argc != 2 || strcmp (argv[1], "foo") != 0)
    {
      fprintf (stderr, "%s: You must run ./erestartsys-trap-debugger instead!\n",
	       argv[0]);
      return 77;
    }
