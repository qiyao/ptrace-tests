/* Test case for PTRACE_SYSCALL at PTRACE_EVENT_{CLONE,FORK,VFORK} stop
   after non-PTRACE_SYSCALL entry to the clone/fork call.
...
test_fork (void)
{
  ...
  switch (fork ())
    ...
    default:			/* parent */
      close (-23);		/* Identifiable syscall number.  */
      _exit (0);
    }
}
...
main (int ac, char *av[])
{
  ...
  /* Start syscall tracing for the fork parent stopped at the
  ...
  /* The test fails if this is the syscall-entry stop for "close (-23);".
