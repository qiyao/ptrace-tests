/* Test the syscall-restart-disable must NOT for rax cut down to 32 bits
   0xfffffe00 = -512 = -ERESTARTSYS with x86_64 debugger and i386 debuggee.
   Test specifically with a machine generated segfault instead of just
   artifical raise().
...
#define DEBUGEE_PATHNAME "./erestartsys-trap-32fails-debuggee"
...
#if !(defined REGISTER_IP && defined REGISTER_ERESTARTSYS)
...
#else	/* REGISTER_IP && REGISTER_ERESTARTSYS */
...
main (int argc, char **argv)
{
  ...
  /* Its size must match `erestartsys-trap-32fails-debuggee.c'.  */
  char buf[0x100];
  size_t buf_have = 0;
  ...
  switch (child)
    ...
    case -1:
      ...
      assert (0);
    case 0:
      execl (DEBUGEE_PATHNAME, "erestartsys-trap-32fails-debuggee", "foo", NULL);
      assert (0);
    default:
      break;
    }
...
# error "Unsupported arch"
  ...
  /* Cut down the restart code ERESTARTSYS (-512) == 0xfffffffffffffe00.  */
  user_orig REGISTER_ERESTARTSYS &= 0xffffffff;
  ...
  /* Return codes are reverted - we require a failure.  */
...
#endif	/* REGISTER_IP && REGISTER_ERESTARTSYS */
